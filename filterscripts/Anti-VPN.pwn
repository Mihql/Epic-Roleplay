...
public MyHttpResponse(playerid, response_code, data[])
{
	...
	{
		format(string, 256, "[PL-RP Notice]: %s(%d) has joined the server.", name, playerid);
	...
	if(response_code == 200)
	{
		if(data[0] == 'Y')
		{
	    	...
	    	SendClientMessage(playerid, 0xFF0000FF, "_________Please disable your VPN and rejoin!_________");
	    	SetTimerEx("DelayedKick", 100, false, "i", playerid);
		}
		if(data[0] == 'N')
		{
			format(string, 256, "[PL-RP Notice]: %s(%d) thank you for joining!", name, playerid);
	    	SendClientMessage(playerid, 0x09F7DFC8, string );
		}
		if(data[0] == 'X')
		{
